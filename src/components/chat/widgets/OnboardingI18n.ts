export const I18N_ONBOARDING_UI: Record<string, Record<string, string>> = {
  english: {
    showAll: "Show All",
    hideAll: "Hide All",
    beforeFood: "Before Food",
    afterFood: "After Food",
    foodFrequency: "Food Frequency",
    on: "ON",
    off: "OFF",
    none: "None",
    extractedMedicationsList: "Extracted Medications List",
    pleaseCheckWhichMedicines: "Please check which medicines to keep in your list:",
    review: "Review",
    confirmSelection: "Confirm Selection",
    addNew: "Add New",
    skipAll: "Skip All",
    verifyTitle: "Verify Information",
    editProfileDetails: "Edit Profile Details",
    selectDateOfBirth: "Select Date of Birth",
    male: "Male",
    female: "Female",
    saveDetails: "Save Details",
    cancel: "Cancel",
    confirmYourProfileDetails: "Confirm your profile details",
    weFoundTwoDifferentProfiles: "We found two different profiles",
    pleaseCheckAndConfirmAllDetails: "Please check and confirm all details below",
    pleaseReviewAndChooseOneYouPrefer: "Please review and choose the one you prefer",
    yourDetails: "Your Details",
    fromDocument: "From Document",
    confirmAndContinue: "Confirm & Continue",
    editDetails: "Edit Details",
    useSocialLogin: "Use Social Login",
    useDocument: "Use Document",
    useEditedInformation: "Use Edited Information",
    editedInformation: "Edited Information",
    editManuallyInstead: "Edit manually instead",
    chooseDate: "Choose Date",
    chooseTime: "Choose Time",
    fromSocialLogin: "From Social Login",
    fromGoogle: "From Google",
    fromFacebook: "From Facebook",
    fromPhone: "From Phone",
    fromEmail: "From Email",
    documentUploaded: "Document Uploaded: ",
    whyAmISeeingThis: "Why am I seeing this?",
    fromApple: "From Apple",
    fromMicrosoft: "From Microsoft",
    morning: "Morning (08:00)",
    noon: "Noon (14:00)",
    night: "Night (20:00)",
    custom: "Custom",
    counter: "Choose {required} times · {selected} of {required} selected",
    duplicate: "This time is already added",
    saveGateError: "Please select exactly {required} time slots",
    nameRequired: "Name is required",
    unitRequired: "Unit is required",
    saveMedicine: "Save Medicine",
    editMedicationDetails: "Please edit the medication details below:",
    processingDocs: "Processing Documents...",
    processingSub: "We are extracting medical details in the background.",
    medicinesExtracted: "Medicines Extracted ({count})",
    reviewExtractedSub: "Review the extracted medicines before confirming.",
    conflictResolution: "Conflict Resolution ({current}/{total})",
    duplicateDetectedSub: "A duplicate was detected for \"{name}\". Please choose how to resolve:",
    finalConfirmation: "Final Confirmation",
    verifySummarySub: "Verify the summary of changes before adding to your vault.",
    medicinesAddedSuccess: "Medicines Added Successfully!",
    successSubtitle: "Your medications and reminder timings have been set up in your vault.",
    keepExisting: "Keep Existing",
    replace: "Replace",
    merge: "Merge",
    discardNewMedicine: "Discard New Medicine",
    confirmAndAddMeds: "Confirm & Add Medicines",
    docsProcessed: "Documents Processed",
    medsExtractedLabel: "Medicines Extracted",
    duplicateConflictsResolved: "Duplicate Conflicts Resolved",
    medsReadyToAdd: "Medicines Ready to Add",
    editMedicine: "Edit Medicine",
    addMedicine: "Add Medication Details",
    medicineName: "Medicine Name",
    medicineType: "Medicine Type",
    dose: "Dosing Intake",
    frequency: "Intake Frequency",
    times: "Intake Schedule",
    prescribedBy: "Prescribed By",
    notes: "Special Instructions / Notes",
    refillAlert: "Refill Alert Notification",
    totalQuantity: "Total Quantity",
    confirmSave: "Confirm & Save",
    edit: "Edit",
    "placeholder.paracetamol": "e.g. Paracetamol",
    "placeholder.qty": "e.g. 30",
    "placeholder.notes": "Special instructions (e.g. Take with warm water)...",
    "medicineType.TABLET": "Tablet",
    "medicineType.CAPSULE": "Capsule",
    "medicineType.SYRUP": "Syrup",
    "medicineType.INJECTION": "Injection",
    "medicineType.DROPS": "Drops",
    "medicineType.SPRAY": "Spray",
    "medicineType.INHALER": "Inhaler",
    "dosePreview.tablet": "Take {count} tablet(s) per intake",
    "dosePreview.capsule": "Take {count} capsule(s) per intake",
    "dosePreview.puff": "Take {count} puff(s) per intake",
    "dosePreview.other": "Take {count} {unit} per intake",
    "frequency.ONCE": "Once",
    "frequency.TWICE": "Twice",
    "frequency.THRICE": "Thrice",

    validating: "Verifying file details...",
    uploading: "Uploading report...",
    queued: "Queued for processing...",
    processing: "Analyzing report details...",
    cancelling: "Cancelling upload...",
    success: "Analysis Complete",
    upload_cancelled: "Upload cancelled",
    retry_count: "Retrying ({attempt}/{max})...",
    page_progress: "Page {current} of {total}",
    eta_hint: "Estimated time: ~45 seconds per page",
    btn_try_again: "Try Again",
    btn_choose_different: "Choose a different file",
    btn_cancel: "Cancel",
    err_network_timeout:
      "Connection timed out. The server took too long to respond.",
    err_server_busy: "Server is busy. Please try again in a moment.",
    err_ocr_failed:
      "Document analysis failed. Please verify it is a valid medical record.",
    err_unsupported_file:
      "Unsupported file type. Please upload a PDF or image (PNG, JPG, WEBP).",
    err_file_too_large: "File is too large. Limit is 50MB.",
    err_server_unreachable:
      "AI server is unreachable. Please check your connection and try again.",
    err_upload_failed:
      "File upload failed. Please check your network connection.",
    err_unexpected_error:
      "An unexpected error occurred during processing. Please try again.",
    aiExtractedIntro: "I reviewed your uploaded reports and extracted the following medicines.",
    aiConfirmIntro: "I've prepared your medicines based on the extracted information. Please review them one last time before adding them to your medication list.",
    aiConflictIntro: "I found some possible medication conflicts that you should review before continuing.",
    aiSuccessIntro: "Your medicines have been successfully added. I've also created reminders for each medication.",
    medicinesFound: "Medicines Found",
    reportNames: "Report Name(s)",
    onboardingCanSkipMessage:
      "Your onboarding is complete! The Skip button is now enabled. You can tap Skip to go directly to the Dashboard and complete any remaining steps later.",
    addAndContinue: "Add & Continue",
    saveAndReview: "Save / Review",
    saveMedicines: "Save Medicines",
    medicineNumber: "Medicine #{n}",
    medicinesReadyToReview: "{count} medicine(s) drafted and ready to review",
    medicineEntryCancelled: "Medicine entry has been cancelled.",
    missingStartDateWarning:
      "One or more selected medicines are missing a Start Date. Please edit them to add a Start Date.",
    pastStartDateWarning:
      "One or more selected medicines have a past Start Date. Please edit them to set a current or future Start Date.",
    knownAllergiesQuestion: "Do you have any known allergies?",
    allergyNo: "No",
    allergyYes: "Yes",
    allergyNotSure: "I'm not sure",
    whatAreYouAllergicTo: "What are you allergic to?",
    allergyPlaceholder: "Enter allergy (e.g. Peanuts, Penicillin)",
    addAnotherAllergy: "+ Add another allergy",
    addAllergy: "Add",
    continueButton: "Continue",
    allergyRequired: "Please add at least one allergy or select 'No'",
    allergyDuplicate: "This allergy is already added",
    allergiesLabel: "Allergies: {list}",
    addYourAllergiesTitle: "Add Your Allergies",
    pleaseAddYourAllergies: "Please add your allergies.",
    addedAllergiesLabel: "Added allergies:",
    allergyInputPlaceholder: "Enter allergy",
    allergy_penicillin: "Penicillin",
    allergy_aspirin: "Aspirin",
    allergy_ibuprofen: "Ibuprofen",
    allergy_dust: "Dust",
    allergy_pollen: "Pollen",
    allergy_peanuts: "Peanuts",
    allergy_shellfish: "Shellfish",
    allergy_latex: "Latex",
  },
  gujarati: {
    showAll: "બધું બતાવો",
    hideAll: "બધું છુપાવો",
    beforeFood: "જમ્યા પહેલા",
    afterFood: "જમ્યા પછી",
    foodFrequency: "ભોજન સાથેનો સમય",
    on: "ચાલુ",
    off: "બંધ",
    none: "કોઈ નહિ",
    extractedMedicationsList: "મેળવેલી દવાઓની યાદી",
    pleaseCheckWhichMedicines: "કૃપા કરીને તપાસો કે કઈ દવાઓ તમારી સૂચિમાં રાખવી છે:",
    review: "સમીક્ષા જરૂરી",
    confirmSelection: "આગળ વધો",
    addNew: "ઉમેરો",
    skipAll: "બધું છોડી દો",
    verifyTitle: "માહિતીની ચકાસણી",
    editProfileDetails: "પ્રોફાઇલ વિગતો સુધારો",
    selectDateOfBirth: "જન્મ તારીખ પસંદ કરો",
    male: "પુરુષ",
    female: "સ્ત્રી",
    saveDetails: "સાચવો",
    cancel: "રદ કરો",
    confirmYourProfileDetails: "પ્રોફાઇલ વિગતોની પુષ્ટિ કરો",
    weFoundTwoDifferentProfiles: "અમને બે અલગ પ્રોફાઇલ મળી છે",
    pleaseCheckAndConfirmAllDetails: "કૃપા કરીને નીચેની બધી વિગતો તપાસો અને પુષ્ટિ કરો",
    pleaseReviewAndChooseOneYouPrefer: "કૃપા કરીને સમીક્ષા કરો અને તમારી પસંદગી પસંદ કરો",
    yourDetails: "તમારી વિગતો",
    fromDocument: "દસ્તાવેજથી",
    confirmAndContinue: "પુષ્ટિ કરો અને ચાલુ રાખો",
    editDetails: "વિગતો સુધારો",
    useSocialLogin: "સોશિયલ લોગિન વાપરો",
    useDocument: "દસ્તાવેજ વાપરો",
    useEditedInformation: "સુધારેલી માહિતી વાપરો",
    editedInformation: "સુધારેલી માહિતી",
    editManuallyInstead: "તેના બદલે વિગતો જાતે સુધારો",
    chooseDate: "તારીખ પસંદ કરો",
    chooseTime: "સમય પસંદ કરો",
    fromSocialLogin: "સોશિયલ લોગિનથી",
    fromGoogle: "Google થી",
    fromFacebook: "Facebook થી",
    fromPhone: "ફોન નંબરથી",
    fromEmail: "ઇમેઇલથી",
    documentUploaded: "દસ્તાવેજ અપલોડ થયો: ",
    whyAmISeeingThis: "હું આ કેમ જોઈ રહ્યો છું?",
    fromApple: "Apple થી",
    fromMicrosoft: "Microsoft થી",
    morning: "સવાર (૦૮:૦૦)",
    noon: "બપોર (૧૪:૦૦)",
    night: "રાત (૨૦:૦૦)",
    custom: "કસ્ટમ સમય",
    counter: "પસંદ કરો {required} વખત · {selected} માંથી {required} પસંદ કરેલ",
    duplicate: "આ સમય અગાઉથી જ ઉમેરેલો છે",
    saveGateError: "કૃપા કરીને બરાબર {required} સમય સ્લોટ પસંદ કરો",
    nameRequired: "દવાનું નામ જરૂરી છે",
    unitRequired: "યુનિટ જરૂરી છે",
    saveMedicine: "દવા સાચવો",
    editMedicationDetails: "કૃપા કરીને નીચે દવાની વિગતોમાં સુધારો કરો:",
    processingDocs: "દસ્તાવેજો પ્રોસેસ થઈ રહ્યા છે...",
    processingSub: "અમે બેકગ્રાઉન્ડમાં તબીબી વિગતો મેળવી રહ્યા છીએ.",
    medicinesExtracted: "મેળવેલી દવાઓ ({count})",
    reviewExtractedSub: "પુષ્ટિ કરતા પહેલા મેળવેલી દવાઓની સમીક્ષા કરો.",
    conflictResolution: "ડુપ્લિકેટ ઉકેલ ({current}/{total})",
    duplicateDetectedSub: "\"{name}\" માટે ડુપ્લિકેટ મળી છે. કૃપા કરીને ઉકેલવા માટે વિકલ્પ પસંદ કરો:",
    finalConfirmation: "અંતિમ પુષ્ટિ",
    verifySummarySub: "તમારા વૉલ્ટમાં ઉમેરતા પહેલા ફેરફારોના સારાંશની ચકાસણી કરો.",
    medicinesAddedSuccess: "દવાઓ સફળતાપૂર્વક ઉમેરવામાં આવી!",
    successSubtitle: "તમારા વૉલ્ટમાં તમારી દવાઓ અને રીમાઇન્ડર સમય સેટ થઈ ગયો છે.",
    keepExisting: "જૂનું રાખો",
    replace: "બદલો",
    merge: "ભેગું કરો",
    discardNewMedicine: "નવી દવા રદ કરો",
    confirmAndAddMeds: "પુષ્ટિ કરો અને દવાઓ ઉમેરો",
    docsProcessed: "પ્રોસેસ થયેલ દસ્તાવેજો",
    medsExtractedLabel: "મેળવેલી દવાઓ",
    duplicateConflictsResolved: "ડુપ્લિકેટ ઉકેલ સારાંશ",
    medsReadyToAdd: "ઉમેરવા માટે તૈયાર દવાઓ",
    editMedicine: "દવા સુધારો",
    addMedicine: "દવાની વિગત ઉમેરો",
    medicineName: "દવાનું નામ",
    medicineType: "દવાનો પ્રકાર",
    dose: "દવાનો ખોરાક (માત્રા)",
    frequency: "લેવાની આવર્તન (દિવસમાં કેટલી વાર)",
    times: "લેવાનો સમય",
    prescribedBy: "પ્રિસ્ક્રાઇબ કરનાર ડોક્ટર",
    notes: "ખાસ સૂચનાઓ / નોંધ",
    refillAlert: "રિફિલ એલર્ટ નોટિફિકેશન",
    totalQuantity: "કુલ જથ્થો",
    confirmSave: "પુષ્ટિ કરો અને સાચવો",
    edit: "સુધારો",
    "placeholder.paracetamol": "દા.ત. પેરાસીટામોલ",
    "placeholder.qty": "દા.ત. 30",
    "placeholder.notes": "ખાસ સૂચનાઓ (દા.ત. નવશેકા પાણી સાથે લેવી)...",
    "medicineType.TABLET": "ટેબ્લેટ (ગોળી)",
    "medicineType.CAPSULE": "કેપ્સ્યુલ",
    "medicineType.SYRUP": "સિરોપ (દવા)",
    "medicineType.INJECTION": "ઇન્જેક્શન",
    "medicineType.DROPS": "ટીપાં",
    "medicineType.SPRAY": "સ્પ્રે",
    "medicineType.INHALER": "ઇનહેલર",
    "dosePreview.tablet": "ખોરાક દીઠ {count} ગોળી(ઓ) લો",
    "dosePreview.capsule": "ખોરાક દીઠ {count} કેપ્સ્યુલ લો",
    "dosePreview.puff": "ખોરાક દીઠ {count} પફ લો",
    "dosePreview.other": "ખોરાક દીઠ {count} {unit} લો",
    "frequency.ONCE": "એક વાર",
    "frequency.TWICE": "બે વાર",
    "frequency.THRICE": "ત્રણ વાર",

    validating: "દસ્તાવેજની વિગતો ચકાસાઈ રહી છે...",
    uploading: "મેડિકલ રિપોર્ટ અપલોડ થઈ રહ્યો છે...",
    queued: "પ્રક્રિયા માટે કતારમાં...",
    processing: "રિપોર્ટ વિગતોનું વિશ્લેષણ કરવામાં આવી રહ્યું છે...",
    cancelling: "અપલોડ રદ કરી રહ્યાં છીએ...",
    success: "વિશ્લેષણ પૂર્ણ",
    upload_cancelled: "અપલોડ રદ કરવામાં આવ્યું",
    retry_count: "ફરી પ્રયાસ કરી રહ્યા છીએ ({attempt}/{max})...",
    page_progress: "પૃષ્ઠ {total} માંથી {current}",
    eta_hint: "અંદાજિત સમય: પૃષ્ઠ દીઠ ~45 સેકન્ડ",
    btn_try_again: "ફરી પ્રયાસ કરો",
    btn_choose_different: "બીજી ફાઈલ પસંદ કરો",
    btn_cancel: "રદ કરો",
    err_network_timeout:
      "જોડાણ સમયસીમા સમાપ્ત થઈ ગઈ. સર્વરે પ્રતિસાદ આપવામાં ઘણો સમય લીધો.",
    err_server_busy: "સર્વર વ્યસ્ત છે. કૃપા કરીને થોડીવાર પછી ફરી પ્રયાસ કરો.",
    err_ocr_failed:
      "દસ્તાવેજ વિશ્લેષણ નિષ્ફળ ગયું. કૃપા કરીને ચકાસો કે તે માન્ય મેડિકલ રેકોર્ડ છે.",
    err_unsupported_file:
      "અસમર્થિત ફાઇલ પ્રકાર. કૃપા કરીને પીડીએફ અથવા છબી (PNG, JPG, WEBP) અપલોડ કરો.",
    err_file_too_large: "ફાઇલ ખૂબ મોટી છે. મર્યાદા 50MB છે.",
    err_server_unreachable:
      "AI સર્વર અગમ્ય છે. કૃપા કરીને તમારું કનેક્શન તપાસો અને ફરી પ્રયાસ કરો.",
    err_upload_failed:
      "ફાઇલ અપલોડ નિષ્ફળ ગઈ. કૃપા કરીને તમારું નેટવર્ક કનેક્શન તપાસો.",
    err_unexpected_error:
      "પ્રક્રિયા દરમિયાન એક અનપેક્ષિત ભૂલ આવી. કૃપા કરીને ફરી પ્રયાસ કરો.",
    aiExtractedIntro: "મેં તમારા અપલોડ કરેલા અહેવાલોની સમીક્ષા કરી અને નીચેની દવાઓ બહાર કાઢી.",
    aiConfirmIntro: "મેં બહાર કાઢેલી માહિતીના આધારે તમારી દવાઓ તૈયાર કરી છે. કૃપા કરીને તમારી દવાઓની સૂચિમાં ઉમેરતા પહેલા છેલ્લી વાર તેમની સમીક્ષા કરો.",
    aiConflictIntro: "મને કેટલીક સંભવિત દવાની વિસંગતતાઓ મળી છે જેની તમારે આગળ વધતા પહેલા સમીક્ષા કરવી જોઈએ.",
    aiSuccessIntro: "તમારી દવાઓ સફળતાપૂર્વક ઉમેરવામાં આવી છે. મેં દરેક દવા માટે રીમાઇન્ડર્સ પણ બનાવ્યા છે.",
    medicinesFound: "દવાઓ મળી",
    reportNames: "અહેવાલ નામ(ઓ)",
    onboardingCanSkipMessage:
      "તમારું ઓનબોર્ડિંગ પૂર્ણ થઈ ગયું છે! હવે SKIP બટન સક્રિય થઈ ગયું છે. તમે SKIP પર ટેપ કરીને સીધા ડેશબોર્ડ પર જઈ શકો છો અને બાકી રહેલા સ્ટેપ્સ પછીથી પણ પૂર્ણ કરી શકો છો.",
    addAndContinue: "ઉમેરો અને ચાલુ રાખો",
    saveAndReview: "સાચવો / સમીક્ષા કરો",
    saveMedicines: "દવાઓ સાચવો",
    medicineNumber: "દવા #{n}",
    medicinesReadyToReview: "{count} દવાઓ તૈયાર છે અને સમીક્ષા માટે ઉપલબ્ધ છે",
    medicineEntryCancelled: "દવા ઉમેરવાની પ્રક્રિયા રદ કરવામાં આવી છે.",
    missingStartDateWarning:
      "એક અથવા વધુ પસંદ કરેલી દવાઓમાં શરૂઆતની તારીખ ખૂટે છે. શરૂઆતની તારીખ ઉમેરવા માટે કૃપા કરીને તેને સંપાદિત કરો.",
    pastStartDateWarning:
      "એક અથવા વધુ પસંદ કરેલી દવાઓમાં શરૂઆતની તારીખ ભૂતકાળની છે. કૃપા કરીને ચાલુ અથવા ભવિષ્યની શરૂઆતની તારીખ સેટ કરવા માટે તેને સંપાદિત કરો.",
    knownAllergiesQuestion: "શું તમને કોઈ જાણીતી એલર્જી છે?",
    allergyNo: "ના",
    allergyYes: "હા",
    allergyNotSure: "મને ખાતરી નથી",
    whatAreYouAllergicTo: "તમને શેની એલર્જી છે?",
    allergyPlaceholder: "એલર્જી દાખલ કરો (દા.ત. મગફળી, પેનિસિલિન)",
    addAnotherAllergy: "+ બીજી એલર્જી ઉમેરો",
    addAllergy: "ઉમેરો",
    continueButton: "આગળ વધો",
    allergyRequired: "કૃપા કરીને ઓછામાં ઓછી એક એલર્જી ઉમેરો અથવા 'ના' પસંદ કરો",
    allergyDuplicate: "આ એલર્જી પહેલેથી ઉમેરેલી છે",
    allergiesLabel: "એલર્જી: {list}",
    addYourAllergiesTitle: "તમારી એલર્જી ઉમેરો",
    pleaseAddYourAllergies: "કૃપા કરીને તમારી એલર્જી ઉમેરો.",
    addedAllergiesLabel: "ઉમેરેલી એલર્જી:",
    allergyInputPlaceholder: "એલર્જી દાખલ કરો",
    allergy_penicillin: "પેનિસિલિન",
    allergy_aspirin: "એસ્પિરિન",
    allergy_ibuprofen: "આઇબ્યુપ્રોફેન",
    allergy_dust: "ધૂળ",
    allergy_pollen: "પરાગરજ",
    allergy_peanuts: "મગફળી",
    allergy_shellfish: "શેલફિશ",
    allergy_latex: "લેટેક્સ",
  },
  hindi: {
    showAll: "सभी दिखाएं",
    hideAll: "सभी छिपाएं",
    beforeFood: "भोजन से पहले",
    afterFood: "भोजन के बाद",
    foodFrequency: "भोजन की आवृत्ति",
    on: "चालू",
    off: "बंद",
    none: "कोई नहीं",
    extractedMedicationsList: "निकाली गई दवाओं की सूची",
    pleaseCheckWhichMedicines: "कृपया जांचें कि कौन सी दवाएं अपनी सूची में रखनी हैं:",
    review: "समीक्षा आवश्यक",
    confirmSelection: "चयन की पुष्टि करें",
    addNew: "नई जोड़ें",
    skipAll: "सभी छोड़ें",
    verifyTitle: "जानकारी सत्यापित करें",
    editProfileDetails: "प्रोफ़ाइल विवरण संपादित करें",
    selectDateOfBirth: "जन्म तिथि चुनें",
    male: "पुरुष",
    female: "महिला",
    saveDetails: "विवरण सहेजें",
    cancel: "रद्द करें",
    confirmYourProfileDetails: "अपने प्रोफ़ाइल विवरण की पुष्टि करें",
    weFoundTwoDifferentProfiles: "हमें दो अलग-अलग प्रोफ़ाइल मिली हैं",
    pleaseCheckAndConfirmAllDetails: "कृपया नीचे दिए गए सभी विवरणों की जांच करें और पुष्टि करें",
    pleaseReviewAndChooseOneYouPrefer: "कृपया समीक्षा करें और अपनी पसंदीदा प्रोफ़ाइल चुनें",
    yourDetails: "आपका विवरण",
    fromDocument: "दस्तावेज़ से",
    confirmAndContinue: "पुष्टि करें और जारी रखें",
    editDetails: "विवरण संपादित करें",
    useSocialLogin: "सोशल लॉगिन का उपयोग करें",
    useDocument: "दस्तावेज़ का उपयोग करें",
    useEditedInformation: "संपादित जानकारी का उपयोग करें",
    editedInformation: "संपादित जानकारी",
    editManuallyInstead: "इसके बजाय मैन्युअल रूप से संपादित करें",
    chooseDate: "तारीख चुनें",
    chooseTime: "समय चुनें",
    fromSocialLogin: "सोशल लॉगिन से",
    fromGoogle: "Google से",
    fromFacebook: "Facebook से",
    fromPhone: "फ़ोन से",
    fromEmail: "ईमेल से",
    documentUploaded: "दस्तावेज़ अपलोड किया गया: ",
    whyAmISeeingThis: "मैं यह क्यों देख रहा हूँ?",
    fromApple: "Apple से",
    fromMicrosoft: "Microsoft से",
    morning: "सुबह (08:00)",
    noon: "दोपहर (14:00)",
    night: "रात (20:00)",
    custom: "कस्टम समय",
    counter: "चुनें {required} बार · {required} में से {selected} चयनित",
    duplicate: "यह समय पहले ही जोड़ा जा चुका है",
    saveGateError: "कृपया ठीक {required} समय स्लॉट चुनें",
    nameRequired: "दवा का नाम आवश्यक है",
    unitRequired: "इकाई आवश्यक है",
    saveMedicine: "दवा सहेजें",
    editMedicationDetails: "कृपया नीचे दवा के विवरण को संपादित करें:",
    processingDocs: "दस्तावेजों को संसाधित किया जा रहा है...",
    processingSub: "हम पृष्ठभूमि में चिकित्सा विवरण निकाल रहे हैं।",
    medicinesExtracted: "निकाली गई दवाएं ({count})",
    reviewExtractedSub: "पुष्टि करने से पहले निकाली गई दवाओं की समीक्षा करें।",
    conflictResolution: "डुप्लिकेट समाधान ({current}/{total})",
    duplicateDetectedSub: "\"{name}\" के लिए एक डुप्लिकेट पाया गया था। कृपया हल करने के लिए चुनें:",
    finalConfirmation: "अंतिम पुष्टि",
    verifySummarySub: "अपने वॉल्ट में जोड़ने से पहले परिवर्तनों के सारांश को सत्यापित करें।",
    medicinesAddedSuccess: "दवाएं सफलतापूर्वक जोड़ी गईं!",
    successSubtitle: "आपके वॉल्ट में आपकी दवाएं और अनुस्मारक समय निर्धारित हो गया है।",
    keepExisting: "मौजूदा रखें",
    replace: "बदलें",
    merge: "मिलाएं",
    discardNewMedicine: "नई दवा त्यागें",
    confirmAndAddMeds: "पुष्टि करें और दवाएं जोड़ें",
    docsProcessed: "संसाधित दस्तावेज़",
    medsExtractedLabel: "निकाली गई दवाएं",
    duplicateConflictsResolved: "डुप्लिकेट समाधान सारांश",
    medsReadyToAdd: "जोड़ने के लिए तैयार दवाएं",
    editMedicine: "दवा संपादित करें",
    addMedicine: "दवा विवरण जोड़ें",
    medicineName: "दवा का नाम",
    medicineType: "दवा का प्रकार",
    dose: "खुराक की मात्रा",
    frequency: "खुराक की आवृत्ति",
    times: "खुराक का समय",
    prescribedBy: "डॉक्टर द्वारा पर्ची",
    notes: "विशेष निर्देश / नोट्स",
    refillAlert: "रिफिल अलर्ट अधिसूचना",
    totalQuantity: "कुल मात्रा",
    confirmSave: "पुष्टि करें और सहेजें",
    edit: "संपादित करें",
    "placeholder.paracetamol": "जैसे: पैरासिटामोल",
    "placeholder.qty": "जैसे: 30",
    "placeholder.notes": "विशेष निर्देश (जैसे: गुनगुने पानी के साथ लें)...",
    "medicineType.TABLET": "टैबलेट",
    "medicineType.CAPSULE": "कैप्सूल",
    "medicineType.SYRUP": "सिरप",
    "medicineType.INJECTION": "इंजेक्शन",
    "medicineType.DROPS": "बूंदें",
    "medicineType.SPRAY": "स्प्रे",
    "medicineType.INHALER": "इनहेलर",
    "dosePreview.tablet": "प्रति खुराक {count} टैबलेट लें",
    "dosePreview.capsule": "प्रति खुराक {count} कैप्सूल लें",
    "dosePreview.puff": "प्रति खुराक {count} पफ लें",
    "dosePreview.other": "प्रति खुराक {count} {unit} लें",
    "frequency.ONCE": "एक बार",
    "frequency.TWICE": "दो बार",
    "frequency.THRICE": "तीन बार",

    validating: "दस्तावेज़ विवरण सत्यापित किया जा रहा है...",
    uploading: "रिपोर्ट अपलोड की जा रही है...",
    queued: "प्रसंस्करण के लिए कतारबद्ध...",
    processing: "रिपोर्ट विवरण का विश्लेषण किया जा रहा है...",
    cancelling: "अपलोड रद्द किया जा रहा है...",
    success: "विश्लेषण पूर्ण",
    upload_cancelled: "अपलोड रद्द कर दिया गया",
    retry_count: "पुनः प्रयास कर रहे हैं ({attempt}/{max})...",
    page_progress: "पृष्ठ {total} में से {current}",
    eta_hint: "अनुमानित समय: लगभग 45 सेकंड प्रति पृष्ठ",
    btn_try_again: "पुनः प्रयास करें",
    btn_choose_different: "दूसरी फ़ाइल चुनें",
    btn_cancel: "रद्द करें",
    err_network_timeout:
      "कनेक्शन का समय समाप्त हो गया। सर्वर ने प्रतिक्रिया देने में बहुत लंबा समय लिया।",
    err_server_busy: "सर्वर व्यस्त है। कृपया कुछ क्षणों में पुनः प्रयास करें।",
    err_ocr_failed:
      "दस्तावेज़ विश्लेषण विफल रहा। कृपया सत्यापित करें कि यह एक वैध मेडिकल रिकॉर्ड है।",
    err_unsupported_file:
      "असमर्थित फ़ाइल प्रकार। कृपया पीडीएफ या छवि (PNG, JPG, WEBP) अपलोड करें।",
    err_file_too_large: "फ़ाइल बहुत बड़ी है। सीमा 50MB है।",
    err_server_unreachable:
      "एआई सर्वर अनुपलब्ध है। कृपया अपना कनेक्शन जांचें और पुनः प्रयास करें।",
    err_upload_failed:
      "फ़ाइल अपलोड विफल रही। कृपया अपना नेटवर्क कनेक्शन जांचें।",
    err_unexpected_error:
      "प्रसंस्करण के दौरान एक अप्रत्याशित त्रुटि हुई। कृपया पुनः प्रयास करें।",
    aiExtractedIntro: "मैंने आपके अपलोड किए गए रिपोर्टों की समीक्षा की और निम्नलिखित दवाएं निकालीं।",
    aiConfirmIntro: "मैंने निकाली गई जानकारी के आधार पर आपकी दवाएं तैयार की हैं। कृपया अपनी दवा सूची में जोड़ने से पहले आखिरी बार उनकी समीक्षा करें।",
    aiConflictIntro: "मुझे कुछ संभावित दवा विसंगतियां मिली हैं जिनकी आपको आगे बढ़ने से पहले समीक्षा करनी चाहिए।",
    aiSuccessIntro: "आसकी दवाएं सफलतापूर्वक जोड़ दी गई हैं। मैंने प्रत्येक दवा के लिए रिमाइंडर भी बनाए हैं।",
    medicinesFound: "दवाएं मिलीं",
    reportNames: "रिपोर्ट नाम",
    onboardingCanSkipMessage:
      "आपका ऑनबोर्डिंग पूरा हो गया है! अब SKIP बटन सक्रिय हो गया है। आप SKIP पर टैप करके सीधे डैशबोर्ड पर जा सकते हैं और बाकी चरण बाद में भी पूरे कर सकते हैं।",
    addAndContinue: "जोड़ें और जारी रखें",
    saveAndReview: "सहेजें / समीक्षा करें",
    saveMedicines: "दवाएँ सहेजें",
    medicineNumber: "दवा #{n}",
    medicinesReadyToReview: "{count} दवाएँ तैयार हैं और समीक्षा के लिए उपलब्ध हैं",
    medicineEntryCancelled: "दवा जोड़ने की प्रक्रिया रद्द कर दी गई है।",
    missingStartDateWarning:
      "एक या अधिक चयनित दवाओं में आरंभ तिथि गायब है। कृपया आरंभ तिथि जोड़ने के लिए उन्हें संपादित करें।",
    pastStartDateWarning:
      "एक या अधिक चयनित दवाओं की आरंभ तिथि बीत चुकी है। कृपया वर्तमान या भविष्य की आरंभ तिथि सेट करने के लिए उन्हें संपादित करें।",
    knownAllergiesQuestion: "क्या आपको कोई ज्ञात एलर्जी है?",
    allergyNo: "नहीं",
    allergyYes: "हाँ",
    allergyNotSure: "मुझे पक्का नहीं पता",
    whatAreYouAllergicTo: "आपको किस चीज से एलर्जी है?",
    allergyPlaceholder: "एलर्जी दर्ज करें (जैसे मूंगफली, पेनिसिलिन)",
    addAnotherAllergy: "+ अन्य एलर्जी जोड़ें",
    addAllergy: "जोड़ें",
    continueButton: "आगे बढ़ें",
    allergyRequired: "कृपया कम से कम एक एलर्जी जोड़ें या 'नहीं' चुनें",
    allergyDuplicate: "यह एलर्जी पहले ही जोड़ी जा चुकी है",
    allergiesLabel: "एलर्जी: {list}",
    addYourAllergiesTitle: "अपनी एलर्जी जोड़ें",
    pleaseAddYourAllergies: "कृपया अपनी एलर्जी जोड़ें।",
    addedAllergiesLabel: "जोड़ी गई एलर्जी:",
    allergyInputPlaceholder: "एलर्जी दर्ज करें",
    allergy_penicillin: "पेनिसिलिन",
    allergy_aspirin: "एस्पिरिन",
    allergy_ibuprofen: "इबुप्रोफेन",
    allergy_dust: "धूल",
    allergy_pollen: "पराग",
    allergy_peanuts: "मूंगफली",
    allergy_shellfish: "शेलफिश",
    allergy_latex: "लेटेक्स",
  },
  marathi: {
    showAll: "सर्व दाखवा",
    hideAll: "सर्व लपवा",
    beforeFood: "जेवणापूर्वी",
    afterFood: "जेवणानंतर",
    foodFrequency: "जेवणाची वारंवारता",
    on: "चालू",
    off: "बंद",
    none: "काहीही नाही",
    extractedMedicationsList: "काढलेल्या औषधांची यादी",
    pleaseCheckWhichMedicines: "कृपया आपली औषध तपासणी करून घ्या आणि कोणती ठेवायची आहेत ते निवडा:",
    review: "पुनरावलोकन आवश्यक",
    confirmSelection: "निवडीची पुष्टी करा",
    addNew: "नवीन जोडा",
    skipAll: "सर्व वगळा",
    verifyTitle: "माहितीची पडताळणी",
    editProfileDetails: "प्रोफाइल तपशील संपादित करा",
    selectDateOfBirth: "जन्म तारीख निवडा",
    male: "पुरुष",
    female: "स्त्री",
    saveDetails: "तपशील जतन करा",
    cancel: "रद्द करा",
    confirmYourProfileDetails: "तुमच्या प्रोफाइल तपशीलाची पुष्टी करा",
    weFoundTwoDifferentProfiles: "आम्हाला दोन भिन्न प्रोफाइल सापडल्या",
    pleaseCheckAndConfirmAllDetails: "कृपया खालील सर्व तपशील तपासा आणि पुष्टी करा",
    pleaseReviewAndChooseOneYouPrefer: "कृपया पुनरावलोकन करा आणि आपल्याला हवे ते निवडा",
    yourDetails: "तुमचे तपशील",
    fromDocument: "दस्तऐवजावरून",
    confirmAndContinue: "पुष्टी करा आणि पुढे जा",
    editDetails: "तपशील सुधारा",
    useSocialLogin: "सोशल लॉगिन वापरा",
    useDocument: "दस्तऐवज वापरा",
    useEditedInformation: "संपादित माहिती वापरा",
    editedInformation: "संपादित माहिती",
    editManuallyInstead: "त्याऐवजी मॅन्युअली संपादन करा",
    chooseDate: "तारीख निवडा",
    chooseTime: "वेळ निवडा",
    fromSocialLogin: "सोशल लॉगिनवरून",
    fromGoogle: "Google वरून",
    fromFacebook: "Facebook वरून",
    fromPhone: "फोनवरून",
    fromEmail: "ईमेलवरून",
    documentUploaded: "दस्तऐवज अपलोड केला: ",
    whyAmISeeingThis: "मला हे का दिसत आहे?",
    fromApple: "Apple वरून",
    fromMicrosoft: "Microsoft वरून",
    morning: "सकाळी (०८:००)",
    noon: "दुपारी (१४:००)",
    night: "रात्री (२०:००)",
    custom: "कस्टम वेळ",
    counter: "निवडा {required} वेळा · {required} पैकी {selected} निवडलेले",
    duplicate: "ही वेळ आधीच जोडली आहे",
    saveGateError: "कृपया नेमके {required} वेळ स्लॉट निवडा",
    nameRequired: "औषधाचे नाव आवश्यक आहे",
    unitRequired: "युनिट आवश्यक आहे",
    saveMedicine: "औषध जतन करा",
    editMedicationDetails: "कृपया खालील औषध तपशील सुधारा:",
    processingDocs: "दस्तऐवज प्रक्रिया करत आहे...",
    processingSub: "आम्ही बॅकग्राउंडमध्ये वैद्यकीय तपशील काढत आहोत.",
    medicinesExtracted: "काढलेली औषधे ({count})",
    reviewExtractedSub: "पुष्टी करण्यापूर्वी काढलेल्या औषधांचे पुनरावलोकन करा.",
    conflictResolution: "डुप्लिकेट निराकरण ({current}/{total})",
    duplicateDetectedSub: "\"{name}\" साठी एक डुप्लिकेट आढळला. कृपया कसे सोडवायचे ते निवडा:",
    finalConfirmation: "अंतिम पुष्टीकरण",
    verifySummarySub: "आपल्या वॉल्टमध्ये जोडण्यापूर्वी बदलांच्या सारांशची पडताळणी करा.",
    medicinesAddedSuccess: "औषधे यशस्वीरित्या जोडली गेली!",
    successSubtitle: "आपल्या वॉल्टमध्ये आपली औषधे आणि स्मरणपत्र वेळ सेट केली गेली आहे.",
    keepExisting: "विद्यमान ठेवा",
    replace: "बदला",
    merge: "एकत्र करा",
    discardNewMedicine: "नवीन औषध नाकारा",
    confirmAndAddMeds: "पुष्टी करा आणि औषधे जोडा",
    docsProcessed: "प्रक्रिया केलेले दस्तऐवज",
    medsExtractedLabel: "काढलेली औषधे",
    duplicateConflictsResolved: "डुप्लिकेट निराकरण सारांश",
    medsReadyToAdd: "जोडण्यासाठी तयार औषधे",
    editMedicine: "औषध संपादित करा",
    addMedicine: "औषध तपशील जोडा",
    medicineName: "औषधाचे नाव",
    medicineType: "औषधाचा प्रकार",
    dose: "डोस प्रमाण",
    frequency: "डोस वारंवारता",
    times: "डोसची वेळ",
    prescribedBy: "डॉक्टरांचे नाव",
    notes: "विशेष सूचना / नोट्स",
    refillAlert: "रिफिल अलर्ट नोटीफिकेशन",
    totalQuantity: "कुक प्रमाण",
    confirmSave: "पुष्टी करा आणि जतन करा",
    edit: "संपादित करा",
    "placeholder.paracetamol": "उदा. पॅरासिटामॉल",
    "placeholder.qty": "उदा. ३०",
    "placeholder.notes": "विशेष सूचना (उदा. कोमट पाण्यासोबत घ्या)...",
    "medicineType.TABLET": "टॅबलेट (गोळी)",
    "medicineType.CAPSULE": "कॅप्सूल",
    "medicineType.SYRUP": "सिरप",
    "medicineType.INJECTION": "इंजेक्शन",
    "medicineType.DROPS": "थेंब",
    "medicineType.SPRAY": "स्प्रे",
    "medicineType.INHALER": "इनहेलर",
    "dosePreview.tablet": "प्रति डोस {count} गोळी घ्या",
    "dosePreview.capsule": "प्रति डोस {count} कॅप्सूल घ्या",
    "dosePreview.puff": "प्रति डोस {count} पफ घ्या",
    "dosePreview.other": "प्रति डोस {count} {unit} घ्या",
    "frequency.ONCE": "एकदा",
    "frequency.TWICE": "दोनदा",
    "frequency.THRICE": "तीनदा",

    validating: "दस्तऐवज तपशील तपासत आहे...",
    uploading: "अहवाल अपलोड होत आहे...",
    queued: "प्रक्रियेसाठी रांगेत...",
    processing: "अहवाल तपशीलांचे विश्लेषण करत आहे...",
    cancelling: "अपलोड रद्द करत आहे...",
    success: "विश्लेषण पूर्ण",
    upload_cancelled: "अपलोड रद्द केले",
    retry_count: "पुन्हा प्रयत्न करत आहे ({attempt}/{max})...",
    page_progress: "पान {total} पैकी {current}",
    eta_hint: "अंदाजे वेळ: प्रति पृष्ठ ~४५ सेकंद",
    btn_try_again: "पुन्हा प्रयत्न करा",
    btn_choose_different: "वेगळी फाईल निवडा",
    btn_cancel: "रद्द करा",
    err_network_timeout:
      "कनेक्शनची वेळ संपली. सर्व्हरने प्रतिसाद देण्यास खूप वेळ घेतला.",
    err_server_busy: "सर्व्हर व्यस्त आहे. कृपया काही वेळात पुन्हा प्रयत्न करा.",
    err_ocr_failed:
      "दस्तऐवज विश्लेषण अयशस्वी झाले. कृपया ते वैध वैद्यकीय रेकॉर्ड असल्याची खात्री करा.",
    err_unsupported_file:
      "असमर्थित फाईल प्रकार. कृपया पीडीएफ किंवा प्रतिमा (PNG, JPG, WEBP) अपलोड करा.",
    err_file_too_large: "फाईल खूप मोठी आहे. मर्यादा 50MB आहे.",
    err_server_unreachable:
      "AI सर्व्हर अनुपलब्ध आहे. कृपया आपले कनेक्शन तपासा आणि पुन्हा प्रयत्न करा.",
    err_upload_failed:
      "फाईल अपलोड अयशस्वी झाली. कृपया आपले नेटवर्क कनेक्शन तपासा.",
    err_unexpected_error:
      "प्रक्रियेदरम्यान अनपेक्षित त्रुटी आली. कृपया पुन्हा प्रयत्न करा.",
    aiExtractedIntro: "मी तुमच्या अपलोड केलेल्या अहवालांचे पुनरावलोकन केले आणि खालील औषधे काढली.",
    aiConfirmIntro: "मी काढलेल्या माहितीच्या आधारे तुमची औषधे तयार केली आहेत. कृपया तुमच्या औषधांच्या यादीमध्ये जोडण्यापूर्वी अंतिम वेळी त्यांचे पुनरावलोकन करा.",
    aiConflictIntro: "मला काही संभाव्य औषध विसंगती आढळल्या आहेत ज्यांचे तुम्ही पुढे जाण्यापूर्वी पुनरावलोकन केले पाहिजे.",
    aiSuccessIntro: "तुमची औषधे यशस्वीरित्या जोडली गेली आहेत. मी प्रत्येक औषधासाठी स्मरणपत्रे देखील तयार केली आहेत.",
    medicinesFound: "औषधे सापडली",
    reportNames: "अहवाल नाव(ये)",
    onboardingCanSkipMessage:
      "तुमचे ऑनबोर्डिंग पूर्ण झाले आहे! आता SKIP बटण सक्रिय झाले आहे. तुम्ही SKIP वर टॅप करून थेट डॅशबोर्डवर जाऊ शकता आणि उर्वरित स्टेप्स नंतरही पूर्ण करू शकता.",
    addAndContinue: "जोडा आणि पुढे सुरू ठेवा",
    saveAndReview: "जतन करा / पुनरावलोकन करा",
    saveMedicines: "औषधे जतन करा",
    medicineNumber: "औषध #{n}",
    medicinesReadyToReview: "{count} औषधे तयार आहेत आणि पुनरावलोकनासाठी उपलब्ध आहेत",
    medicineEntryCancelled: "औषध नोंदणी रद्द केली आहे.",
    missingStartDateWarning:
      "निवडलेल्या औषधांपैकी एक किंवा अधिक औषधांना सुरू होण्याची तारीख नाही. सुरू होण्याची तारीख जोडण्यासाठी कृपया त्यांना संपादित करा.",
    pastStartDateWarning:
      "निवडलेल्या औषधांपैकी एक किंवा अधिक औषधांना भूतकाळातील सुरू होण्याची तारीख आहे. कृपया चालू किंवा भविष्यातील सुरू होण्याची तारीख सेट करण्यासाठी त्यांना संपादित करा.",
    knownAllergiesQuestion: "तुम्हाला कोणतीही ज्ञात ऍलर्जी आहे का?",
    allergyNo: "नाही",
    allergyYes: "होय",
    allergyNotSure: "मला नक्की माहित नाही",
    whatAreYouAllergicTo: "तुम्हाला कशाची ऍलर्जी आहे?",
    allergyPlaceholder: "ऍलर्जी प्रविष्ट करा (उदा. शेंगदाणे, पेनिसिलिन)",
    addAnotherAllergy: "+ आणखी एक ऍलर्जी जोडा",
    addAllergy: "जोडा",
    continueButton: "पुढे सुरू ठेवा",
    allergyRequired: "कृपया किमान एक ऍलर्जी जोडा किंवा 'नाही' निवडा",
    allergyDuplicate: "ही ऍलर्जी आधीच जोडलेली आहे",
    allergiesLabel: "ऍलर्जी: {list}",
    addYourAllergiesTitle: "तुमची ऍलर्जी जोडा",
    pleaseAddYourAllergies: "कृपया तुमची ऍलर्जी जोडा.",
    addedAllergiesLabel: "जोडलेली ऍलर्जी:",
    allergyInputPlaceholder: "ऍलर्जी प्रविष्ट करा",
    allergy_penicillin: "पेनिसिलिन",
    allergy_aspirin: "ॲस्पिरिन",
    allergy_ibuprofen: "आयबुप्रोफेन",
    allergy_dust: "धूळ",
    allergy_pollen: "परागकण",
    allergy_peanuts: "शेंगदाणे",
    allergy_shellfish: "शेलफিশ",
    allergy_latex: "लेटेक्स",
  },
  tamil: {
    showAll: "அனைத்தையும் காட்டு",
    hideAll: "அனைத்தையும் மறை",
    beforeFood: "உணவுக்கு முன்",
    afterFood: "உணவுக்கு பின்",
    foodFrequency: "உணவு அதிர்வெண்",
    on: "ஆன்",
    off: "ஆஃப்",
    none: "ஏதுமில்லை",
    extractedMedicationsList: "பிரித்தெடுக்கப்பட்ட மருந்துகளின் பட்டியல்",
    pleaseCheckWhichMedicines: "தயவுசெய்து உங்கள் பட்டியலில் வைக்க வேண்டிய மருந்துகளைச் சரிபார்க்கவும்:",
    review: "மதிப்பாய்வு தேவை",
    confirmSelection: "தேர்வை உறுதிப்படுத்துக",
    addNew: "புதியதைச் சேர்",
    skipAll: "அனைத்தையும் தவிர்",
    verifyTitle: "தகவலைச் சரிபார்க்கவும்",
    editProfileDetails: "சுயவிவர விவரங்களைத் திருத்தவும்",
    selectDateOfBirth: "பிறந்த தேதியைத் தேர்ந்தெடுக்கவும்",
    male: "ஆண்",
    female: "பெண்",
    saveDetails: "விவரங்களைச் சேமிக்கவும்",
    cancel: "ரத்துசெய்",
    confirmYourProfileDetails: "உங்கள் சுயவிவர விவரங்களை உறுதிப்படுத்தவும்",
    weFoundTwoDifferentProfiles: "நாங்கள் இரண்டு வெவ்வேறு சுயவிவரங்களைக் கண்டறிந்துள்ளோம்",
    pleaseCheckAndConfirmAllDetails: "தயவுசெய்து கீழே உள்ள அனைத்து விவரங்களையும் சரிபார்த்து உறுதிப்படுத்தவும்",
    pleaseReviewAndChooseOneYouPrefer: "தயவுசெய்து மதிப்பாய்வு செய்து நீங்கள் விரும்பும் ஒன்றை தேர்ந்தெடுக்கவும்",
    yourDetails: "உங்கள் விவரங்கள்",
    fromDocument: "ஆவணத்திலிருந்து",
    confirmAndContinue: "உறுதிப்படுத்தித் தொடரவும்",
    editDetails: "விவரங்களைத் திருத்தவும்",
    useSocialLogin: "சமூக உள்நுழைவைப் பயன்படுத்தவும்",
    useDocument: "ஆவணத்தைப் பயன்படுத்தவும்",
    useEditedInformation: "திருத்தப்பட்ட தகவலைப் பயன்படுத்தவும்",
    editedInformation: "திருத்தப்பட்ட தகவல்",
    editManuallyInstead: "அதற்கு பதிலாக கைமுறையாக திருத்தவும்",
    chooseDate: "தேதியைத் தேர்ந்தெடு",
    chooseTime: "நேரத்தைத் தேர்ந்தெடு",
    fromSocialLogin: "சமூக உள்நுழைவிலிருந்து",
    fromGoogle: "Google இலிருந்து",
    fromFacebook: "Facebook இலிருந்து",
    fromPhone: "தொலைபேசியிலிருந்து",
    fromEmail: "மின்னஞ்சலிலிருந்து",
    documentUploaded: "ஆவணம் பதிவேற்றப்பட்டது: ",
    whyAmISeeingThis: "இதை நான் ஏன் பார்க்கிறேன்?",
    fromApple: "Apple இலிருந்து",
    fromMicrosoft: "Microsoft இலிருந்து",
    morning: "காலை (08:00)",
    noon: "மதியம் (14:00)",
    night: "இரவு (20:00)",
    custom: "தனிப்பயன் நேரம்",
    counter: "{required} நேரங்களைத் தேர்ந்தெடுக்கவும் · {required}-ல் {selected} தேர்ந்தெடுக்கப்பட்டது",
    duplicate: "இந்த நேரம் ஏற்கனவே சேர்க்கப்பட்டுள்ளது",
    saveGateError: "சரியாக {required} நேரங்களை தேர்ந்தெடுக்கவும்",
    nameRequired: "மருந்து பெயர் தேவை",
    unitRequired: "அலகு தேவை",
    saveMedicine: "மருந்தைச் சேமிக்கவும்",
    editMedicationDetails: "தயவுசெய்து கீழே உள்ள மருந்து விவரங்களைத் திருத்தவும்:",
    processingDocs: "ஆவணங்கள் செயலாக்கப்படுகின்றன...",
    processingSub: "பின்னணியில் மருத்துவ விவரங்களை பிரித்தெடுக்கிறோம்.",
    medicinesExtracted: "பிரித்தெடுக்கப்பட்ட மருந்துகள் ({count})",
    reviewExtractedSub: "உறுதிப்படுத்துவதற்கு முன் பிரித்தெடுக்கப்பட்ட மருந்துகளை மதிப்பாய்வு செய்யவும்.",
    conflictResolution: "நகல் தீர்வு ({current}/{total})",
    duplicateDetectedSub: "\"{name}\"-க்கு ஒரு நகல் கண்டறியப்பட்டது. தயவுசெய்து எப்படி தீர்ப்பது என்பதை தேர்வு செய்யவும்:",
    finalConfirmation: "இறுதி உறுதிப்படுத்தல்",
    verifySummarySub: "உங்கள் வால்ட்டில் சேர்ப்பதற்கு முன் மாற்றங்களின் சுருக்கத்தை சரிபார்க்கவும்.",
    medicinesAddedSuccess: "மருந்துகள் வெற்றிகரமாக சேர்க்கப்பட்டன!",
    successSubtitle: "உங்கள் மருந்துகளும் நினைவூட்டல் நேரங்களும் உங்கள் வால்ட்டில் அமைக்கப்பட்டுள்ளன.",
    keepExisting: "ஏற்கனவே உள்ளதை வை",
    replace: "மாற்று",
    merge: "இணைக்கவும்",
    discardNewMedicine: "புதிய மருந்தை நிராகரி",
    confirmAndAddMeds: "உறுதிப்படுத்தி மருந்துகளைச் சேர்க்கவும்",
    docsProcessed: "செயலாக்கப்பட்ட ஆவணங்கள்",
    medsExtractedLabel: "பிரித்தெடுக்கப்பட்ட மருந்துகள்",
    duplicateConflictsResolved: "நகல் தீர்வு சுருக்கம்",
    medsReadyToAdd: "சேர்க்கத் தயாராக உள்ள மருந்துகள்",
    editMedicine: "மருந்தைத் திருத்தவும்",
    addMedicine: "மருந்து விவரங்களைச் சேர்க்கவும்",
    medicineName: "மருந்து பெயர்",
    medicineType: "மருந்து வகை",
    dose: "மருந்தின் அளவு",
    frequency: "மருந்து உட்கொள்ளும் அதிர்வெண்",
    times: "மருந்து உட்கொள்ளும் நேரம்",
    prescribedBy: "பரிந்துரைத்தவர்",
    notes: "சிறப்பு வழிமுறைகள் / குறிப்புகள்",
    refillAlert: "மீள்நிரப்பு எச்சரிக்கை அறிவிப்பு",
    totalQuantity: "மொத்த அளவு",
    confirmSave: "உறுதிசெய்து சேமிக்கவும்",
    edit: "திருத்து",
    "placeholder.paracetamol": "உதாரணமாக: பாராசிட்டமால்",
    "placeholder.qty": "உதாரணமாக: 30",
    "placeholder.notes": "சிறப்பு வழிமுறைகள் (உதாரணமாக: வெதுவெதுப்பான நீருடன் உட்கொள்ளவும்)...",
    "medicineType.TABLET": "மாத்திரை",
    "medicineType.CAPSULE": "காப்ஸ்யூல்",
    "medicineType.SYRUP": "சிரப் (திரவ மருந்து)",
    "medicineType.INJECTION": "ஊசி",
    "medicineType.DROPS": "சொட்டு மருந்து",
    "medicineType.SPRAY": "ஸ்ப்ரே",
    "medicineType.INHALER": "இன்ஹேலர்",
    "dosePreview.tablet": "ஒரு முறைக்கு {count} மாத்திரை(கள்) உட்கொள்ளவும்",
    "dosePreview.capsule": "ஒரு முறைக்கு {count} காப்ஸ்யூல் உட்கொள்ளவும்",
    "dosePreview.puff": "ஒரு முறைக்கு {count} பஃப்ஸ் உட்கொள்ளவும்",
    "dosePreview.other": "ஒரு முறைக்கு {count} {unit} உட்கொள்ளவும்",
    "frequency.ONCE": "ஒரு முறை",
    "frequency.TWICE": "இரு முறை",
    "frequency.THRICE": "மூன்று முறை",

    validating: "ஆவண விவரங்கள் சரிபார்க்கப்படுகின்றன...",
    uploading: "அறிக்கை பதிவேற்றப்படுகிறது...",
    queued: "செயலாக்கத்திற்காக வரிசைப்படுத்தப்பட்டுள்ளது...",
    processing: "அறிக்கை விவரங்கள் பகுப்பாய்வு செய்யப்படுகின்றன...",
    cancelling: "பதிவேற்றம் ரத்து செய்யப்படுகிறது...",
    success: "பகுப்பாய்வு முடிந்தது",
    upload_cancelled: "பதிவேற்றம் ரத்து செய்யப்பட்டது",
    retry_count: "மீண்டும் முயற்சிக்கிறது ({attempt}/{max})...",
    page_progress: "பக்கம் {total}-ல் {current}",
    eta_hint: "மதிப்பிடப்பட்ட நேரம்: ஒரு பக்கத்திற்கு ~45 வினாடிகள்",
    btn_try_again: "மீண்டும் முயற்சி செய்",
    btn_choose_different: "வேறு கோப்பைத் தேர்ந்தெடுக்கவும்",
    btn_cancel: "ரத்து செய்",
    err_network_timeout:
      "இணைப்பு காலாவதியானது. பதிலளிக்க சேவையகம் அதிக நேரம் எடுத்துக்கொண்டது.",
    err_server_busy:
      "சேவையகம் பிஸியாக உள்ளது. சற்று நேரத்தில் மீண்டும் முயற்சிக்கவும்.",
    err_ocr_failed:
      "ஆவண பகுப்பாய்வு தோல்வியடைந்தது. இது ஒரு செல்லுபடியாகும் மருத்துவ அறிக்கை என்பதை உறுதிப்படுத்தவும்.",
    err_unsupported_file:
      "ஆதரிக்கப்படாத கோப்பு வகை. பிடிஎஃப் அல்லது படம் (PNG, JPG, WEBP) பதிவேற்றவும்.",
    err_file_too_large: "கோப்பு மிகப்பெரியது. வரம்பு 50MB ஆகும்.",
    err_server_unreachable:
      "AI சேவையகத்தை இணைக்க முடியவில்லை. உங்கள் இணைப்பைச் சரிபார்த்து மீண்டும் முயற்சிக்கவும்.",
    err_upload_failed:
      "கோப்பு பதிவேற்றம் தோல்வியடைந்தது. உங்கள் இணைய இணைப்பைச் சரிபார்க்கவும்.",
    err_unexpected_error:
      "செயலாக்கத்தின் போது எதிர்பாராத பிழை ஏற்பட்டது. மீண்டும் முயற்சிக்கவும்.",
    aiExtractedIntro: "நான் நீங்கள் பதிவேற்றிய அறிக்கைகளை மதிப்பாய்வு செய்து பின்வரும் மருந்துகளை பிரித்தெடுத்தேன்.",
    aiConfirmIntro: "பிரித்தெடுக்கப்பட்ட தகவலின் அடிப்படையில் உங்கள் மருந்துகளைத் தயாரித்துள்ளேன். உங்கள் மருந்துப் பட்டியலில் சேர்ப்பதற்கு முன் கடைசி முறை அவற்றை மதிப்பாய்வு செய்யவும்.",
    aiConflictIntro: "தொடர்வதற்கு முன் நீங்கள் மதிப்பாய்வு செய்ய வேண்டிய சில சாத்தியமான மருந்து முரண்பாடுகளைக் கண்டறிந்தேன்.",
    aiSuccessIntro: "உங்கள் மருந்துகள் வெற்றிகரமாக சேர்க்கப்பட்டுள்ளன. ஒவ்வொரு மருந்திற்கும் நினைவூட்டல்களையும் உருவாக்கியுள்ளேன்.",
    medicinesFound: "மருந்துகள் கண்டறியப்பட்டன",
    reportNames: "அறிக்கை பெயர்(கள்)",
    onboardingCanSkipMessage:
      "உங்கள் ஆன்போர்டிங் முடிந்துவிட்டது! இப்போது SKIP பொத்தான் செயல்படுத்தப்பட்டுள்ளது. SKIP என்பதைத் தட்டி நேரடியாக Dashboard-க்கு செல்லலாம் மற்றும் மீதமுள்ள படிகளை பின்னரும் முடிக்கலாம்.",
    addAndContinue: "சேர்த்து தொடரவும்",
    saveAndReview: "சேமி / மதிப்பாய்வு",
    saveMedicines: "மருந்துகளைச் சேமி",
    medicineNumber: "மருந்து #{n}",
    medicinesReadyToReview: "{count} மருந்துகள் வரைவு செய்யப்பட்டு மதிப்பாய்வுக்கு தயார்",
    medicineEntryCancelled: "மருந்து பதிவு ரத்து செய்யப்பட்டது.",
    missingStartDateWarning:
      "தேர்ந்தெடுக்கப்பட்ட ஒன்று அல்லது அதற்கு மேற்பட்ட மருந்துகளுக்கு தொடக்க தேதி இல்லை. தொடக்க தேதியை சேர்க்க அவற்றை திருத்தவும்.",
    pastStartDateWarning:
      "தேர்ந்தெடுக்கப்பட்ட ஒன்று அல்லது அதற்கு மேற்பட்ட மருந்துகளுக்கு கடந்த கால தொடக்க தேதி உள்ளது. தற்போதைய அல்லது எதிர்கால தொடக்க தேதியை அமைக்க அவற்றை திருத்தவும்.",
    knownAllergiesQuestion: "உங்களுக்கு ஏதேனும் ஒவ்வாமை (அலர்ஜி) இருக்கிறதா?",
    allergyNo: "இல்லை",
    allergyYes: "ஆம்",
    allergyNotSure: "எனக்கு உறுதியாகத் தெரியவில்லை",
    whatAreYouAllergicTo: "உங்களுக்கு எதனால் ஒவ்வாமை ஏற்படுகிறது?",
    allergyPlaceholder: "ஒவ்வாமையை உள்ளிடவும் (எ.கா. வேர்க்கடலை, பென்சிலின்)",
    addAnotherAllergy: "+ மற்றொரு ஒவ்வாமையைச் சேர்க்கவும்",
    addAllergy: "சேர்",
    continueButton: "தொடரவும்",
    allergyRequired: "குறைந்தது ஒரு ஒவ்வாமையையாவது சேர்க்கவும் அல்லது 'இல்லை' என்பதைத் தேர்ந்தெடுக்கவும்",
    allergyDuplicate: "இந்த ஒவ்வாமை ஏற்கனவே சேர்க்கப்பட்டுள்ளது",
    allergiesLabel: "ஒவ்வாமை: {list}",
    addYourAllergiesTitle: "உங்கள் ஒவ்வாமைகளைச் சேர்க்கவும்",
    pleaseAddYourAllergies: "உங்கள் ஒவ்வாமைகளைச் சேர்க்கவும்.",
    addedAllergiesLabel: "சேர்க்கப்பட்ட ஒவ்வாமைகள்:",
    allergyInputPlaceholder: "ஒவ்வாமையை உள்ளிடவும்",
    allergy_penicillin: "பென்சிலின்",
    allergy_aspirin: "ஆஸ்பிரின்",
    allergy_ibuprofen: "இபுப்ரோஃபென்",
    allergy_dust: "தூசி",
    allergy_pollen: "மகரந்தம்",
    allergy_peanuts: "வேர்க்கடலை",
    allergy_shellfish: "சிப்பி மீன்",
    allergy_latex: "லேடெக்ஸ்",
  }
};
