...
export const MedicationExtractionWizard: React.FC<MedicationExtractionWizardProps> = ({
  preferredLang,
  isDark,
  theme,
}) => {
  ...
  // Auto transition from processing to results
  useEffect(() => {
    if (chatWizardState.step === "processing" && isAllTerminal && chatWizardState.jobIds.length > 0 && !isLoadingResults) {
      const getResults = async () => {
        ...
        try {
          ...
          console.error("Failed to load extracted medicines inside chat:", err);
          setChatWizardState((prev) => ({
            ...prev,
            step: "results",
            extractedMedicines: [],
  ...
  const resolveCurrentConflict = (
    ...
    setChatWizardState((prev) => {
      ...
      if (resolution === "replace") {
        ...
        // Keep Existing: do not add the new one, do not change existing
      } else if (resolution === "remove_new") {
        // Same as keep
      }
  ...
  const renderProcessingStep = () => {
    return (
      <StepWrapper>
        <StepTitle isDark={isDark}>{t("processingDocs")}</StepTitle>
        <StepSubtitle>{t("processingSub")}</StepSubtitle>
        {jobList.map((job) => {
          ...
          return (
            <DocProgressRow key={job.jobId} isDark={isDark}>
              <DocHeaderRow>
                <Ionicons name="document-text-outline" size={18} color="#0f766e" style={{ marginRight: 8 }} />
                <DocName numberOfLines={1} isDark={isDark}>
                  {chatWizardState.filesInfo.find((f) => f.jobId === job.jobId)?.fileName || `Document ${job.jobId.slice(0, 8)}`}
                </DocName>
    ...
    if (!hasMeds) {
      return (
        <EmptyWrapper>
          ...
          <ActionButton onPress={resetChatWizard}>
  ...
  return (
    <ContainerCard isDark={isDark}>
      {chatWizardState.step === "processing" && renderProcessingStep()}
      {chatWizardState.step === "results" && renderResultsStep()}
      {chatWizardState.step === "conflicts" && renderConflictsStep()}
      {chatWizardState.step === "summary" && renderSummaryStep()}
      {chatWizardState.step === "completed" && renderCompletedStep()}
...
export default MedicationExtractionWizard;

/* --- Styled Components --- */

const ContainerCard = styled.View<{ isDark: boolean }>`
  width: 100%;
  border-radius: 20px;
  background-color: ${(props: any) => (props.isDark ? "rgba(30, 41, 59, 0.7)" : "#ffffff")};
  border-width: 1px;
  border-color: ${(props: any) => (props.isDark ? "rgba(255,255,255,0.08)" : "#e2e8f0")};
  padding: 16px;
  margin-vertical: 8px;
  shadow-color: #000;
  shadow-opacity: 0.05;
