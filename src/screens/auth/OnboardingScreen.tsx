...
const ONBOARDING_I18N: Record<string, Record<string, string>> = {
  ...
  gujarati: {
    ...
    err_unexpected_error: "પ્રક્રિયા દરમિયાન એક અનપેક્ષિત ભૂલ આવી. કૃપા કરીને ફરી પ્રયાસ કરો."
  },
  hindi: {
...
export default function OnboardingScreen() {
  ...
  useEffect(() => {
    const resumePendingJob = async () => {
      try {
        ...
        if (pendingDocId && pendingDocId !== "null" && pendingDocId !== "undefined" && pendingDocId.trim() !== "") {
          const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
          if (!uuidRegex.test(pendingDocId)) {
            console.log("[ONBOARDING] Invalid document ID format in storage, clearing:", pendingDocId);
          ...
          try {
            ...
            if (resData) {
              if (resData.status === "done") {
                ...
                pollPendingOcrJob(pendingDocId);
              }
            }
          } catch (err: any) {
            console.warn("[ONBOARDING] Failed to query pending document status on server:", err.message);
            if (err?.response?.status === 404) {
              console.log("[ONBOARDING] Document not found on server (404), clearing storage ID:", pendingDocId);
              await AsyncStorage.removeItem("onboarding_pending_document_id");
            }
          }
        } else {
    ...
    if (userData) {
      ...
      const fetchOnboardingHistory = async () => {
        ...
        try {
          ...
          if (chatSessionId && Array.isArray(historyItems) && historyItems.length > 0) {
            console.log("[ONBOARDING] Replaying", historyItems.length, "historical messages.");
            const mappedMessages: Message[] = historyItems.map((dbMsg: any) => ({
              ...(dbMsg.metadata || {}),
              id: dbMsg.id,
              role: dbMsg.role,
          ...
          console.error("[ONBOARDING] Failed to load onboarding history:", error);
  ...
  const renderOptions = (
    ...
    const isComplexStep =
      activeMsg.action === "RESOLVE_PROFILE_SOURCE" ||
      activeMsg.action === "ASK_UPLOAD_OR_SKIP" ||
      activeMsg.action === "MEDICINE_OPTIONS" ||
      activeMsg.action === "ADD_MEDICINE" ||
      activeMsg.action === "EDIT_MEDICINE" ||
      activeMsg.action === "REVIEW_MEDICINES_LIST" ||
      activeMsg.action === "CONFIRM_MEDICINE";

    const { chosenVal, chosenLabel } = findHistoricalUserReply(messages, activeMsg.id, false);

    const uiT = (key: string) => {
      const lang = preferredLang || "english";
      const dict = ONBOARDING_I18N[lang.toLowerCase()] || ONBOARDING_I18N.english;
    ...
    if (
      ...
      const displayDate = isHistorical && chosenLabel ? chosenLabel : uiT("chooseDate");
      return (
        <View style={styles.actionRow} pointerEvents={isHistorical ? "none" : "auto"}>
          <TouchableOpacity
    ...
    if (activeMsg.action === "ASK_MEDICINE_SCHEDULE") {
      const displayTime = isHistorical && chosenLabel ? chosenLabel : uiT("chooseTime");
      return (
        <View style={styles.actionRow} pointerEvents={isHistorical ? "none" : "auto"}>
          <TouchableOpacity
            disabled={isHistorical}
    ...
    if (activeMsg.action === "ADD_MEDICINE" || activeMsg.action === "EDIT_MEDICINE" || (activeMedicineToEdit && !isHistorical)) {
      const med = (isHistorical ? activeMsg.medicine : (activeMedicineToEdit || activeMsg.medicine)) || {};
    ...
    if (activeMsg.action === "REVIEW_MEDICINES_LIST") {
      ...
      return (
        <ReviewMedicinesListCard
          localMedicines={isHistorical ? (activeMsg.medicines || []) : localMedicines}
      ...
      return (
        <View style={styles.chipRow} pointerEvents={isHistorical ? "none" : "auto"}>
          {activeMsg.options.map((opt) => {
            ...
            const isChosen = isHistorical && (
              (chosenVal && String(value).toLowerCase() === String(chosenVal).toLowerCase()) ||
              (chosenLabel && String(label).toLowerCase() === String(chosenLabel).toLowerCase())
            );
            const isUnchosen = isHistorical && !isChosen;
          ...
          <View style={styles.listWrapper}>
            <FlatList
              ...
              renderItem={({ item }) => {
                ...
                const options = renderOptions(item, isLast, isHistorical);
                const isJson =
