...
function AddMedicineCard({
  ...
  const getDosePreviewText = () => {
    ...
    if (formType === "TABLET") {
      ...
      return isGuj
        ? `${fracLabel} ગોળી પ્રત્યેક ડોઝ દીઠ`
        : `${fracLabel} tablet(s) per intake`;
    }
    if (formType === "CAPSULE") {
      return isGuj
        ? `${formCount} કેપ્સ્યુલ પ્રત્યેક ડોઝ દીઠ`
        : `${formCount} capsule(s) per intake`;
    }
    if (formType === "SPRAY" || formType === "INHALER") {
      return isGuj
        ? `${formVal} પફ પ્રત્યેક ડોઝ દીઠ`
        : `${formVal} puff(s) per intake`;
    }
    return isGuj
      ? `${formVal} ${formUnit} પ્રત્યેક ડોઝ દીઠ`
      : `${formVal} ${formUnit} per intake`;
  };
    ...
    >
      ...
      <View style={styles.inputGroup}>
        ...
        >
          {[
            ...
            const label =
              preferredLang === "gujarati"
                ? currentLabelLabels[tItem].gu
                : currentLabelLabels[tItem].en;
            return (
              <TouchableOpacity
                key={tItem}
                style={[
      ...
      <View style={styles.inputGroup}>
        ...
        <View style={styles.chipRow}>
          {["BEFORE_FOOD", "AFTER_FOOD"].map((f) => {
            const label = f === "BEFORE_FOOD" 
              ? (preferredLang === "gujarati" ? "જમ્યા પહેલા" : "Before Food")
              : (preferredLang === "gujarati" ? "જમ્યા પછી" : "After Food");
            return (
              <TouchableOpacity
                key={f}
                style={[
                  ...
                  {
                    backgroundColor:
                      formFoodFreq === f ? theme.colors.primary : isDark ? "#334155" : "#f1f5f9",
                    paddingVertical: 8,
                    paddingHorizontal: 16,
                    borderRadius: 10,
                    marginRight: 8,
              ...
              >
                <Text style={[styles.freqChipText, { color: formFoodFreq === f ? "#ffffff" : theme.colors.textPrimary, fontWeight: "bold" }]}>
                  {label}
                </Text>
              </TouchableOpacity>
            );
      ...
      <View style={styles.inputGroup}>
        ...
        <TextInput
          ...
          placeholder={
            preferredLang === "gujarati" ? "ડૉક્ટર નું નામ" : "Doctor's Name"
          }
          placeholderTextColor={isDark ? "#64748b" : "#94a3b8"}
        />
      </View>
...
export default function OnboardingScreen() {
  ...
  const handleSuccessfulOcr = async (pollResult: any, fileName: string) => {
    ...
    sendMessage("DOCUMENT_UPLOADED", newState, "Document Uploaded: " + fileName);
  };
    ...
    return new Promise((resolve, reject) => {
      const interval = setInterval(async () => {
        ...
        try {
          ...
          console.log("[ONBOARDING] Poll OCR Status:", jobData?.status, "Stage:", jobData?.stage);
  ...
  const uploadSelectedFile = async (fileToUpload = selectedFile) => {
    ...
    try {
      ...
      if (!fileKey) {
        throw new Error("Failed to upload document: No fileKey returned.");
      }

      setUploadProgress("Initiating OCR...");
      // Step 2: Run OCR
      if (isUploadCancelledRef.current) throw new Error("UPLOAD_CANCELLED");
      const runOcrRes = await runOcr({ fileKey });
      const jobId = runOcrRes.data?.jobId;
      if (!jobId) {
      ...
      // Step 3: Poll Status
      if (isUploadCancelledRef.current) throw new Error("UPLOAD_CANCELLED");
      const pollResult = await pollOcrStatus(fileKey);
      ...
      if (pollResult && pollResult.stage === "COMPLETED") {
        ...
        throw new Error("OCR failed or did not complete successfully. Stage: " + pollResult?.stage);
      }
    } catch (error: any) {
      console.error("[Onboarding] Document processing failed:", error);

      if (error.message === "UPLOAD_CANCELLED") {
        console.log("[Onboarding] Document upload cancelled by user.");
        setUploadProgress(null);
        return;
      }
      ...
      if (isTimeoutOrNetworkError && fileKey) {
        setUploadProgress(
          "Analyzing report details...",
        );
        try {
          const pollResult = await pollOcrStatus(fileKey);
          if (pollResult && (pollResult.status === "COMPLETED" || pollResult.stage === "COMPLETED")) {
            setUploadProgress("Saving Document...");
            await addDocument({
              s3Key: fileKey,
              fileName: fileToUpload.name,
          ...
          console.error(
            "[Onboarding] Polling OCR status failed:",
            pollErr,
          );
        }
      }
            ...
            <View style={{ flex: 1 }}>
              ...
              >
                {activeMsg.title ||
                  (mode === "CONFIRM"
                    ? preferredLang === "gujarati"
                      ? "પ્રોફાઇલ વિગતોની પુષ્ટિ કરો"
                      : "Confirm your profile details"
                    : preferredLang === "gujarati"
                      ? "અમને બે અલગ પ્રોફાઇલ મળી છે"
    ...
    if (activeMsg.action === "ADD_MEDICINE" || activeMedicineToEdit) {
      ...
      const handleSave = (updatedMed: any) => {
        if (isEditingLocal) {
          setLocalMedicines((prev) =>
            prev.map((m) =>
              m.client_med_id === med.client_med_id
                ? {
                    ...m,
                    ...updatedMed,
                    subtitle:
    ...
    if (activeMsg.action === "REVIEW_MEDICINES_LIST") {
      const handleConfirm = (checkedMeds: string[]) => {
        sendMessage(
          ...
          state,
          preferredLang === "gujarati"
            ? "પસંદ કરેલી દવાઓની પુષ્ટિ કરો"
            : "Confirm Selected",
        );
      };
      const handleAddNew = () => {
        sendMessage(
          JSON.stringify({ addNew: true }),
          state,
          preferredLang === "gujarati" ? "નવી દવા ઉમેરો" : "Add New",
        );
      };
      const handleSkipAll = () => {
        sendMessage(
          JSON.stringify({ skipAll: true }),
          state,
          preferredLang === "gujarati" ? "બધી દવાઓ છોડી દો" : "Skip All",
        );
      };
    ...
    if (activeMsg.action === "CONFIRM_MEDICINE") {
      const handleConfirm = () => {
        sendMessage(
          JSON.stringify({ confirmed: true }),
          state,
          preferredLang === "gujarati" ? "હા, યોગ્ય છે" : "Yes, Correct",
        );
      };
      const handleEdit = () => {
        sendMessage(
          JSON.stringify({ edit: true }),
          state,
          preferredLang === "gujarati" ? "સુધારો" : "Edit",
        );
      };
          ...
          {(state.medicinesToAdd || []).map((med, index) => (
            ...
            >
              ...
              {med.medicationSchedule &&
                Array.isArray(med.medicationSchedule) &&
                med.medicationSchedule.length > 0 && (
                  <Text
                    style={{
                      color: theme.colors.textSecondary,
                      fontSize: 13,
              ...
              renderItem={({ item }) => {
                ...
                return (
                  <View style={{ width: "100%" }}>
                    ...
                    {isAi && options !== null && (
                      <View 
                        style={[styles.optionsWrapper, { opacity: isLast ? 1 : 0.5 }]} 
